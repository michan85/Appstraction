...
import appstraction.app.style.css.CSSelly;
import appstraction.app.style.css.Combinator;
import appstraction.app.style.css.CssSelector;
...
public class NodeSelector {
	 ...
	 */
	protected void walkDescendantsIteratively(ArrayList<Node> nodes, CssSelector cssSelector, List<Node> result) {
		while (!nodes.isEmpty()) {
...
//					walk(node, cssSelector, result);
			...
			case ADJACENT_SIBLING:
				Node node = NodeUtils.getNextSiblingElement(rootNode);
				if (node != null) {
					selectAndAdd(node, cssSelector, result);
				}
				break;
			case GENERAL_SIBLING:
				node = rootNode;
				while (true) {
					node = NodeUtils.getNextSiblingElement(rootNode);
					if (node == null) {
						break;
					}
					selectAndAdd(node, cssSelector, result);
